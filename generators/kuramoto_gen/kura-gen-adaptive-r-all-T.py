...
makeDir(os.path.join(os.getcwd(),dirforstim))
...
for number in numbers:
    ...
    #bpms = np.geomspace(72,120, num=2)
    ...
    audio_dir = os.path.join(thedir, "stimuli_" + str(number))
    ...
    plotdir = os.path.join(thedir, "stimuli_" + str(number), "plots")
    
    # print out all the stimuli strings that will be generated    
    if os.path.exists(os.path.join(thedir, "stimuli_" + str(number))) == False:
        print('making dirs for', thedir + "stimuli_" + str(number))
        os.mkdir(os.path.join(thedir, "stimuli_" + str(number)))
        os.mkdir(os.path.join(thedir, "stimuli_" + str(number)," mp3s"))
        os.mkdir(os.path.join(thedir, "stimuli_" + str(number),"phases"))
        os.mkdir(os.path.join(thedir, "stimuli_" + str(number),"trigs"))
        os.mkdir(os.path.join(thedir, "stimuli_" + str(number), "phases", "ang"))
        os.mkdir(os.path.join(thedir, "stimuli_" + str(number), "phases","pc"))
        os.mkdir(os.path.join(thedir, "stimuli_" + str(number), "phases", "init_freqs"))
        os.mkdir(os.path.join(thedir, "stimuli_" + str(number), "phases", "init_phases"))
        os.mkdir(os.path.join(thedir, "stimuli_" + str(number), "phases", "center-bpm"))
        os.mkdir(os.path.join(thedir, "stimuli_" + str(number), "phases","beat-windows"))
    ...
    for bpm in bpms:
        for sync_, kn_, rtar in zip(syncs, kn_coeffs, R_target):
            ...
            #### change the timbre oc each osc by changing sr over linear range 0.95-1.05
            ...
            for x in range(totaliters):
                ...
                for i in range(len(p)):
                    if p[i] < p_[i]: # check for zero crossing
                        ...
                        audiobuffer[pos:(pos+samples[i].size)] = audiobuffer[pos:(pos+samples[i].size)] + samples[i]
            ...
            home = os.path.join(os.getcwd(), dirforstim)
            ...
            # save output audio
            sf.write(os.path.join(home, "stimuli_" + str(number), sync_tempo + ".wav"), audiobuffer_stereo.T, samplerate=22050) 
            ...
            np.save(os.path.join(home, "stimuli_" +  str(number), "phases", sync_tempo + ".npy"), triggerbuffer)        
            # save trigs 
            np.save(os.path.join(home, "stimuli_" +  str(number), "trigs", sync_tempo + ".npy"), trigsbuffer)        
            # save the phase coherence
            np.savetxt(os.path.join(home, "stimuli_" + str(number), "phases","pc",sync_tempo + ".txt"), phasecoherence, delimiter=',')
            # save the avg angle
            np.savetxt(os.path.join(home, "stimuli_" +  str(number), "phases", "ang", sync_tempo + ".txt"), avgphase, delimiter=',')
            # save center freq, init phases, init freqs        
            np.savetxt(os.path.join(home, "stimuli_" +  str(number), "phases", "center-bpm", sync_tempo + ".txt"), np.array([round(freq_bpm,2)]), delimiter=',')        
            np.savetxt(os.path.join(home, "stimuli_" +  str(number), "phases", "init_phases", sync_tempo + ".txt"), init_phases, delimiter=',')
            np.savetxt(os.path.join(home, "stimuli_" +  str(number), "phases", "init_freqs", sync_tempo + ".txt"), init_freqs, delimiter=',')
    ...
    def getBeatBinsFromPsi(filenames, savePlots=False):
        ...
        for i, filen in enumerate(filenames):
            ...
            filepath = os.path.join(dirforstim, 'stimuli_' + cond[-1])
            zcs = np.load(os.path.join(filepath, "phases", cond + ".npy" ), allow_pickle=True)
            psi = np.loadtxt(os.path.join(filepath, "phases", "ang", cond + ".txt"), delimiter=",") 
            y, sr_ = librosa.load(os.path.join(filepath, cond + ".wav"), sr=audio_sr)
        ...
        plt.savefig(os.path.join(plotdir, 'beat-windows', timetag + ".png"), dpi=150)
    ...
    for j,snd in enumerate(thebatch):
        ...
        # v2 by center bpm  
        tempo_cond = np.loadtxt(os.path.join(audio_dir, "phases", "center-bpm", snd + ".txt"), delimiter=',')        
    ...
    for snd in thebatch:
        ...
        np.savetxt(os.path.join(home, "stimuli_" +  str(number), "phases", "beat-windows", thesnd + ".txt"), beat_bins_cropped[snd], delimiter=',')
        
        beat_25_sample_cutoff[snd] = beat_bins_cropped[snd][-1]
        #print beat_23_sample_cutoff[snd]
        stimpath = os.path.join(audio_dir, snd + ".wav")
        #print stimpath
    ...
    for i,snd in enumerate(thebatch):
        ...
        stimpath = os.path.join(thedir, "stimuli_" + str(number), snd + ".wav")    
        y, sr_ = librosa.load(stimpath)
    ...
    def plotCOP(batch):
        ...
        timetag = str(datetime.datetime.now())
        try:
            plt.savefig(os.path.join(plotdir, 'cop/', timetag + '.png'), dpi=150) 
        except IOError:
    ...
    plt.savefig(os.path.join(plotdir, "sorted-by-R.png"),dpi=150)
    ############### save a .csv file of the list 
    import csv 
    with open(os.path.join(plotdir, 'R-ascending-new-25.csv'), 'w') as file:
