...
number = 1 # which version number to make, NB: have to change and rerun block below 
bigtime1 = time.time()

bpms = np.geomspace(60, 120, num=6) # bpms from 60 - 120 bpm on log scale
...
dirforstim = "/stim-no-timbre-all/" # where to put audio at root
...
for snd in thebatch:
    ...
    beat_bins_cropped[snd] = beatbins_filtered[snd][:26]
