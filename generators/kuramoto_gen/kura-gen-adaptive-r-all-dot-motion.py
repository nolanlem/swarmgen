...
# helper functions
def accumulateR(ph, num_active_oscs):
#    function to  complex order params
    # r = 0
    r = np.sum(np.exp(ph[:num_active_oscs]*1j))
    r = r/num_active_oscs # divide by number of active oscillators
...
def updatePhases(ph, freq, orderparams, kn, num_active_oscs):
    # 
    rang, rmag = orderparams
    pang = kn[:num_active_oscs]*rmag*np.sin(rang-ph[:num_active_oscs])
    
    ph[:num_active_oscs] = ph[:num_active_oscs] + freq[:num_active_oscs] + pang # for active oscs
    ph[num_active_oscs:] = ph[num_active_oscs:] + freq[num_active_oscs:] # for non-active oscs
    ph = ph%(2*np.pi)
    # return the phases
    return ph
...
numbers = [1] # which version number to make, NB: have to change and rerun block below 
dir_to_put_stimuli = "/stim-dot-motion/"
...
for number in numbers:
    ...
    syncs = ['strong' for elem in range(20)] # sync conds   
    kn_coeffs = [0.3 for elem in range(20)] # initial kn
    R_target = [0.99999 for elem in range(20)]
    numoscs_active = [elem for elem in range(20)]
    kn_scalar = 0.01 # update increment for each 
    adaptive_window = 10 # number of frames to check R in gen model sr, 10 frames=0.5 sec  
    ...
    for bpm in bpms:
        for sync_, kn_, rtar, active_oscs in zip(syncs, kn_coeffs, R_target, numoscs_active):
            ...
            for x in range(totaliters):
                #print 'kn: ', kn
                complexorderparams = accumulateR(p, active_oscs)
                tempR = complexorderparams[1]
                
                R_mean_arr.append(tempR) 
                
                # don't adapt kn for strong and none conds                
                np.put(kn, np.arange(active_oscs + 1), 1.2*np.ones(active_oscs + 1))
                ...
                ######## UPDATE PHASES ###########
                p_ = np.copy(p) # p[x-1]
                p = updatePhases(p, freq, complexorderparams, kn, active_oscs) #p[x]
            ...
            sync_tempo = '_'.join([str(active_oscs), sync, tempo, str(number)])
            optimal_kns[sync_tempo] = kn
            center_period[sync_tempo] = 1./(freq_bpm/60.)
    
            # save output audio
    
            sf.write(home + "stimuli_" + str(number) + "/" + sync_tempo + ".wav", audiobuffer_stereo.T, samplerate=int(audio_sr))
